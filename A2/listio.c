...
int processStrings(struct dataHeader *header)
{
	...
	pythonProcess(header);

	return SUCCESS;
}

int pythonProcess(struct dataHeader *header)
{
	/*If header is null, return failure*/
	if (header == NULL)
	{
	...
	strcpy(command1, "chmod u+x a2python.py");
	system(command1);
	strcpy(command2, "python a2python.py &" );
	system(command2);
	...
	/*Wait 10 seconds for python to finish processing the strings*/
	sleep(10);
...
void argValidate(int argc, char * argv[])
{
	/*If the user does not enter command line arguements, exit*/
	if (argc != 2)
	{
		printf("Please enter 2 arguements. First the executable, then the filename. \n");
		printf("For example './a2 <filename>\n");
        /*User must add 2 arguements, so if they don't, exit*/
        exit(0);
	}
}
